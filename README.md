...
本專案旨在為莆仙語使用者提供完整、準確的輸入法解決方案，支援兩套羅馬字系統：
...
```
輸入：putian          → 候選：莆田 pou²leng²
輸入：hinghwa         → 候選：興化 hing¹ngua⁴
輸入：siabue          → 候選：食飯 sia²mue⁵
```

類普拼式輸入（方便初學者）：

```
輸入：hv              → 候選：魚 hy²（使用 v 代替 y）
...
**範例**：
- 輸入 `baq` → 只顯示第2調的候選詞：`bá`（爸）
- 輸入 `bav` → 只顯示第3調的候選詞：`bâ`（罷）
