...
- **大小寫智能處理**：支援首字母大寫輸入（如 `Inggio` → `I̍ng-giô̤ⁿ`），用於句首或專有名詞
- **漢語拼音反查**：用 `` ` `` 開啟反查功能
...
```
輸入：b    → 候選：ba, báⁿ, ba̍h, ...
輸入：pc   → 候選：Pó-chéng 莆田
輸入：buc  → 候選：Báⁿ-uā-ci̍ 平話字
```

### 大小寫輸入（智能處理）

純興化平話字模式支援首字母大寫輸入，用於句首或專有名詞：

```
輸入：Inggio    → 候選：I̍ng-giô̤ⁿ（句首或專有名詞）
輸入：inggio    → 候選：i̍ng-giô̤ⁿ（一般文本）
輸入：Singgann  → 候選：Sing-gā̤ⁿ（地名：新縣）
```

**使用說明**：
- **小寫輸入**：輸出小寫平話字（一般文本）
- **首字母大寫**：輸出首字母大寫的平話字（句首、專有名詞）
- 詞庫中所有詞條均為小寫，系統會根據您的輸入自動調整輸出大小寫
...
```
# 漢字版輸出
輸入：`putian`  → 顯示：莆田 pó-chéng

# 純平話字版輸出（平話字為主，漢字為註釋）
輸入：`putian`  → 顯示：pó-chéng (莆田)
```
